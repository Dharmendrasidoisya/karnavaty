...
public partial class apllication_details : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                EditData(Request.QueryString["aid"].ToString());
                BindBreadcrumbs(Request.Params["aid"].ToString());
            }
            else
            {
                EditData("");
    ...
    private void BindBreadcrumbs(string CID)
    {
        try
        {
            ...
            ds = db.fetchds("SELECT * FROM APPLICATIONSEO_MASTER WHERE CID=" + CID + "");
    ...
    private void EditData(string ID)
    {
        ...
        {
            DataSet ds = new DataSet();
            ds = db.fetchds("SELECT * FROM EVENT_MASTER WHERE ID=" + ID + "");
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
    ...
    private void Productspe(string pid)
    {
        ...
        {
            DataSet ds = new DataSet();
            ds = db.fetchds("SELECT * FROM DESCRIPTION_MASTER WHERE PID=" + pid + " AND STATUS=1 ");
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
